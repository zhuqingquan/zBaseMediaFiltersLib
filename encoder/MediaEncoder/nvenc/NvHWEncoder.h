...
public:
	 ...
	 *	@name		ProcessOutput
	 *	@brief		获取到编码成功数据
	 *	@param[in]	const EncodeBuffer * pEncodeBuffer 编码器输入输出的显存buffer
	 *	@param[in]	uint8_t * outputBuffer 用于保存获取到的编码成功后的数据
	 *	@param[in,out]	uint32_t * bufferLen 传入outputBuffer内存块的长度，传出获取到的数据的长度
	 *	@return		NVENCSTATUS NV_ENC_SUCCESS--成功 其他--失败
	 **/
	NVENCSTATUS                                          ProcessOutput(const EncodeBuffer *pEncodeBuffer, uint8_t* outputBuffer, uint32_t* bufferLen, uint64_t* pts, NV_ENC_PIC_TYPE* picType);
