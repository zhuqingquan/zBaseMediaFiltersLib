...
NVENCSTATUS CNvHWEncoder::ProcessOutput( const EncodeBuffer *pEncodeBuffer, uint8_t* outputBuffer, uint32_t* bufferLen, uint64_t* pts, NV_ENC_PIC_TYPE* picType )
{
	NVENCSTATUS nvStatus = NV_ENC_SUCCESS;

	if(outputBuffer==NULL || bufferLen<=0)
	{
		return NV_ENC_ERR_INVALID_PARAM;
	}
	...
	if (nvStatus == NV_ENC_SUCCESS)
	{
		// fixme 如果传入的outputBuffer内存块大小不够，该如何处理？？？
		memcpy(outputBuffer, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
		*pts = lockBitstreamData.outputTimeStamp;
		*picType = lockBitstreamData.pictureType;
		*bufferLen = lockBitstreamData.bitstreamSizeInBytes;
		//fwrite(lockBitstreamData.bitstreamBufferPtr, 1, lockBitstreamData.bitstreamSizeInBytes, m_fOutput);
		nvStatus = m_pEncodeAPI->nvEncUnlockBitstream(m_hEncoder, pEncodeBuffer->stOutputBfr.hBitstreamBuffer);
	}
	else
